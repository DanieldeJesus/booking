...
namespace BookingAPIAlten.Application.Services
{
    public class ReservationAppService : IReservationAppService
    {
        ...
        public async Task<IEnumerable<ReservationViewModel>> GetReservsRooms()
        {
            return ReservationViewModel.FromTo(await _reservationRepository.GetReservsRooms());
        }

        public async Task<IEnumerable<RoomViewModel>> GetAvailableRoom(DateTime? from, DateTime? to)
        {
            return RoomViewModel.FromTo(await _roomRepository.GetAvailableRoom(from, to));
        }
