...
namespace BookingAPIAlten.Data.Repository
{
    public class ReservationRepository: IReservationRepository
    {
        ...
        public async Task<IEnumerable<Reservation>> GetReservsRooms()
        {
            var sql = $@" SELECT
	                             tr.id,
            ...
            return AppendBookings(await _reservationDB.Connection.QueryAsync<Reservation>(sql));
        }
        ...
        public async Task<Reservation> GetReserveName(string name)
        {
            ...
            return AppendBooking(await _reservationDB.Connection.QueryFirstOrDefaultAsync<Reservation>(sql));
        }
        ...
        private Reservation AppendBooking(Reservation reservation)
        {
            if (reservation != null)
            {
                var sql = $@" SELECT
        ...
        private IEnumerable<Reservation> AppendBookings(IEnumerable<Reservation> reservations)
        {
            if (reservations.Count() > 0)
            {
                foreach (var res in reservations)
                {
                    var sql = $@" SELECT
	                                 ...
	                                 tb.booked_to AS BookedTo
                                FROM tb_bookings tb
                               WHERE tb.reservation_id = {res.Id}
                                 AND tb.status IN(1,2) ";

                    res.ReservationRoom = _reservationDB.Connection.QueryFirstOrDefault<Booking>(sql);
                }                
            }
